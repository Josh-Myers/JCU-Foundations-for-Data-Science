...
This analysis used the variables: State, Month, Year, Day of the Week, Time of Day, Bus Involvement, Heavy Rigid Truck Involvement, Articultaed Truck Involvement, Road User, Gender, and Age. Other variables were removed from the dataset using dplyr's 'select' function. The variable State was coded as an orded factor with levels set to decreasing State population size using the 'factor' function. Day of the Week was also coded as an ordered factor from Monday to Sunday. Gender, Bus Involvement, Heavy Rigid Truck Involvement and Articulated Truck involvement were coded as binary factors using the 'as.factor' funtion in base R. Road User was coded as an unordered factor using 'as.factor', and Month was set to be an integer using 'as.integer' in base R.  

State, Month, Year, Day of the Week, Bus Involvement, Articulated Truck Involvement had no missing values. Time of day had 40 missing, Road User 77 missing, Gender 22 missing, and age 84 missing, which were imputed using multiple imputation using the 'mice' function from the *mice* package. Time

Diagnositic plots of imputed values looked satisfactory, and are included in Appendix A. 
